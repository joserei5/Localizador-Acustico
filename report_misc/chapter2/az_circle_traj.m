...
% trajectory along a circle of radius CIRC_R
CIRC_R = +2;
[spk.loc, AOA_t] = speaker_circle2D(div.size, rec.loc, CIRC_R, d.pts, 90);
...
% compare results
% forwards semi-circle
SPK_CIRCLE1 = spk.loc(1:round(end/2),2);
AOA_CIRCLE1 = AOA(1:round(end/2));
AOA_CIRCLE1_t = transpose(AOA_t(1:round(end/2)));
results_p1 = figure;
plot(SPK_CIRCLE1, AOA_CIRCLE1,'r')
hold on
plot(SPK_CIRCLE1, AOA_CIRCLE1_t,'k')
xlabel('Y-axis position (m)')
ylabel('Theta / Azimuth (º)')
legend('algorithm AOA','correct AOA')

% backwards semi-circle
SPK_CIRCLE2 = spk.loc(round(end/2)+1:end,2);
AOA_CIRCLE2 = AOA(round(end/2)+1:end);
AOA_CIRCLE2_t = transpose(AOA_t(round(end/2)+1:end));
results_p2 = figure;
plot(SPK_CIRCLE2, AOA_CIRCLE2,'r')
hold on
plot(SPK_CIRCLE2, AOA_CIRCLE2_t,'k')
xlabel('Y-axis position (m)')
ylabel('Theta / Azimuth (º)')
legend('algorithm AOA','correct AOA')

% theta resolution
% semi-circle analysis only
AOA_s = [0; diff(AOA_CIRCLE1)];
AOA_s_unique = AOA_s(AOA_s~= 0);
AOA_sl = length(AOA_s_unique);

s_v = 1;
s_flag = 0;
for i=1:round(d.pts/2)
    if AOA_s(i) == 0
...
ax1 = axes(t);
ax2 = axes(t);
plot(ax1, SPK_CIRCLE1, AOA_s,'b')
xlabel(ax1,'Y-axis position (m)')
ylabel(ax1,'Theta resolution (º)')
plot(ax2, AOA_CIRCLE1_t ,AOA_s,'b')
ax2.XAxisLocation = 'top';
ax2.YTick = [];
ax2.XTick = round(AOA_t(1),-1):10:round(AOA_t(end),-1);
xlabel(ax2,'Azimuth / Theta (º)')

% detection error of the forwards semi-circle
AOA_e = AOA_CIRCLE1_t - AOA_CIRCLE1;
th_error_p = figure;
t = tiledlayout(1,1);
ax1 = axes(t);
ax2 = axes(t);
plot(ax1, SPK_CIRCLE1, AOA_e,'b')
xlabel(ax1,'Y-axis position (m)')
ylabel(ax1,'Theta error (º)')
plot(ax2, AOA_CIRCLE1_t ,AOA_e,'b')
ax2.XAxisLocation = 'top';
ax2.YTick = [];
ax2.XTick = round(AOA_t(1),-1):10:round(AOA_t(end),-1);
xlabel(ax2,'Azimuth / Theta (º)')